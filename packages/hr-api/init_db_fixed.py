    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("""
            create table if not exists public.performance_reviews (
                ...
                ai_decision text,
                rating integer,                    -- 1..5; consumed by reports / digital-twin / org-AI (matches migrations/20260630)
                ...
                created_at timestamptz default now(),
                constraint performance_reviews_org_employee_cycle_key unique (org_id, employee_id, cycle)
            )
        """))

        # =========================================================
        # COMPANY BOOTSTRAP + AMOUNT-TIER APPROVALS + COMP CYCLE
        # (mirrors migrations/20260709_orgs_approval_comp.sql; keeps a fresh
        #  create_all DB in sync with a fully-migrated one). user_profiles is
        #  an ORM model, so it is already created by create_all above.
        # =========================================================
        await conn.execute(text("""
