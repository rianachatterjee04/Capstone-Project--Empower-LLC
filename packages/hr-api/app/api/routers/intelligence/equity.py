...
async def fairness(actor: Actor = Depends(require_org), db: AsyncSession = Depends(db_session)) -> Dict[str, Any]:
    ...
    try:
        ...
        # equity_shares column is optional in the demo schema. Rather than
        # 500, surface the absence cleanly so the UI can render an empty state.
        try:
            await db.rollback()
        except Exception:
            pass
        return {
            "equity_fairness_score": 1.0,
            "interpretation": "Equity tracking not enabled (no equity_shares column on employees)",
            "departments": {},
            "analysis": None,
