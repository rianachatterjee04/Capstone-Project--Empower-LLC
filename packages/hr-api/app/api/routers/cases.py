...
async def add_evidence(case_id: str, payload: dict, actor: Actor = Depends(require_org), db: AsyncSession = Depends(db_session)):
    ...
    await db.execute(text("""
        ...
        values (:org, :cid, :uid, :desc, cast(:meta as jsonb))
    """), {
        "org": actor.org_id,
        "cid": case_id,
        "uid": actor.user_id,
...
async def findings(case_id: str, payload: dict, actor: Actor = Depends(require_org), db: AsyncSession = Depends(db_session)):
    ...
    _updated = await db.execute(text("""
        update public.cases
        set findings=cast(:findings as jsonb), status='findings_recorded'
        where org_id=:org and id=:cid
    """), {
        "org": actor.org_id,
        "cid": case_id,
...
async def close_case(case_id: str, payload: dict, actor: Actor = Depends(require_org), db: AsyncSession = Depends(db_session)):
    ...
    """), {
        "org": actor.org_id,
        "cid": case_id,
        # A case closed with no reason recorded is a compliance gap: the record
        # says it was closed and cannot say why.
        "reason": required_field(payload, "reason", what="why this case is being closed")
    })
    if _updated.rowcount == 0:
        # The WHERE clause is org-scoped, so zero rows means the id does not
        # belong to this organisation (or does not exist). Continuing wrote an
