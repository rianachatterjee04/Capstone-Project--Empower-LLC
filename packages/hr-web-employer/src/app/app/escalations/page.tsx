...
export default function EscalationsPage() {
  ...
  async function createRule() {
    let condition_dsl: any;
    try {
      condition_dsl = JSON.parse(condition);
    } catch {
      setRuleError("Condition DSL is not valid JSON.");
      return;
    }
    setRuleError(null);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-black/10 p-4 space-y-3">
          ...
          {ruleError ? <div className="text-xs text-red-600">{ruleError}</div> : null}
        ...
        <div className="rounded-2xl border border-black/10 p-4 space-y-3">
          ...
          <div className="text-xs text-black/60">
            The evaluator creates escalations for open cases and raises the level when one is overdue. Escalations are recorded here; sending the notification is not connected yet.
          </div>
