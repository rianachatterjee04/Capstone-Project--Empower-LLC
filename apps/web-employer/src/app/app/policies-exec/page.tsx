...
export default function PolicyExecPage() {
  ...
  async function parse() {
    const r = await apiPost<ParseResponse>("/policies2/parse", {
  ...
  async function create() {
    const r = await apiPost<CreateResponse>("/policies2/create", {
      name,
      policy_text: policyText,
      scope: "org",
    });
  ...
  async function dryRun() {
    const r = await apiPost<DryRunResponse>("/policies2/dry-run", {
      policy_text: policyText,
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="text-sm text-black/60">
          This is the demo that beats Workday: policies become executable SLAs +
          escalations.
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-black/10 p-4 space-y-3">
        ...
        <textarea
          className="w-full rounded-xl border border-black/10 p-3 font-mono text-xs min-h-[200px]"
          value={policyText}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-black/10 p-4">
          ...
          <pre className="mt-3 text-xs bg-black/5 rounded-xl p-3 overflow-auto">
