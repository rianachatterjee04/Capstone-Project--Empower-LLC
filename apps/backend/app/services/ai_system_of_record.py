...
async def upsert_chunk(db: AsyncSession, org_id: UUID, namespace: str, content: str, metadata: Dict[str, Any]) -> str:
    ...
    res = await db.execute(text("""
        ...
        values (:org_id, :namespace, :content, :metadata::jsonb, :embedding)
        returning id
    """), {
        "org_id": str(org_id),
        "namespace": namespace,
...
async def log_decision(db: AsyncSession, org_id: UUID, actor_user_id: Optional[UUID], actor_role: Optional[str],
    ...
    res = await db.execute(text("""
        ...
        values (:org_id, :actor_user_id, :actor_role, :decision_type, :entity_type, :entity_id, :input::jsonb, :output::jsonb, :model)
        returning id
    """), {
        "org_id": str(org_id),
