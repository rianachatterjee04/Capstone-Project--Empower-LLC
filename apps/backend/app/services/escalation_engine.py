...
async def ensure_case_escalations(db: AsyncSession, org_id: UUID) -> int:
    ...
    cases = (await db.execute(select(Case).where(Case.org_id == org_id, Case.status.in_(["open","in_review"])))).scalars().all()
    ...
    for c in cases:
        for r in rules:
            ...
            q = text("""
              ...
              values (:org_id, 'case', :entity_id, :rule_id, 0, 'open', :due_at)
...
async def escalate_overdue(db: AsyncSession, org_id: UUID) -> int:
    ...
    q = text("""
      ...
      where org_id = :org_id and status = 'open' and due_at < now()
      returning id, level
    """)
